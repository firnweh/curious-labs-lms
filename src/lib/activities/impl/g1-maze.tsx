...
/* ──────────────────────────────────────────────────────────────────────────
 * Maze Runner 🌀 — a JUNIOR (Grade 1, age ~6) CODING activity: SEQUENCING.
 * A mouse 🐭 sits at START. A flag 🚩 waits at FINISH across a 5×5 maze with
 * a few walls 🧱. The child taps FOUR big arrow cards ⬆️⬇️⬅️➡️ to plan a path
 * — each tap APPENDS to a visible "program strip" — then taps GO ▶ to send the
 * mouse hopping one cell per step. Land on the flag → big celebration +
 * onComplete({passed:true,stars:3}) once. Bump a wall or the edge → a friendly
 * "bonk" 🙈, the mouse hops home, no scolding. Understood from VISUALS alone —
 * no reading needed. Touch-first, big targets, deterministic, always winnable.
 *
 * The one intended path (6 steps): up,up,right,right,up,up.
 * ────────────────────────────────────────────────────────────────────────── */
...
// Intended path: (0,4)→(0,2)→(2,2)→(2,0). All walls sit OFF that trail and
// gently fence in the wrong turns so the clear road stays obvious.
...
type Outcome =
  ...
  | { type: "wall" }
  | { type: "edge" }
  | { type: "short" }; // path ended before reaching the flag (still fine)
...
function simulate(steps: Dir[]): SimResult {
  ...
  for (const d of steps) {
    ...
    if (isWall(nc, nr)) {
      return { trail, outcome: { type: "wall" } };
    }
    c = nc;
    r = nr;
    trail.push({ c, r });
...
export default function MazeRunner({ onComplete }: ActivityProps) {
  ...
  const go = useCallback(() => {
    if (running || won) return;
    if (steps.length === 0) {
      // Empty isn't a "mistake" — a gentle bonk nudge, then settle.
      setPhase("bonk");
      ...
      if (i >= sim.trail.length) {
        ...
        if (out.type === "win") {
          ...
          if (!reportedRef.current) {
            ...
            onComplete({ passed: true, stars: 3, detail: "You reached the flag! 🚩" });
          }
        } else {
          // Friendly bonk at the stopping point, then hop home. No harsh fail.
          setPhase("bonk");
          if (out.type === "wall" || out.type === "edge") {
            onComplete({
              passed: false,
              detail:
                out.type === "wall"
                  ? "Bonk! A wall stopped the mouse — try a new path."
                  : "Whoops — the mouse hopped off the maze. Try again!",
            });
          }
    ...
    timerRef.current = setTimeout(tick, STEP_MS);
  }, [running, won, steps, clearTimer, resetMouse, onComplete]);
    ...
    <div className="flex w-full flex-col items-center gap-3 font-mono text-ink">
      ...
      <div className="panel relative w-full max-w-[420px] overflow-hidden rounded-2xl border border-line p-2">
        <svg
          ...
          role="img"
          aria-label="A 5 by 5 maze with a mouse and a flag to reach"
        >
          <defs>
          ...
          <g
            style={{
              ...
              animation: won ? "gm-pop 0.7s ease-out" : undefined,
            }}
          >
            <text
              x={cx(GOAL.c)}
          ...
          >
            <g
              style={{
                ...
                animation: bonking
                  ? "gm-wobble 0.5s ease-in-out"
                  : running
                    ? "gm-hop 0.45s ease-in-out infinite"
                    : won
                      ? "gm-pop 0.6s ease-out"
                      : undefined,
              }}
            >
              <circle
      ...
      {/* ── Program strip: the arrows planned so far, in order ── */}
      <div
